...
pipeline = PaperToPatentPipeline(
    output_dir="./output/",
    patent_type="cn",
    reference_docx="template.docx",  # 可选：学习已有专利样式
    bw_figures=True,                  # 黑白图
    verbose=True,
)
...
PDF 论文 ──→ PyMuPDF 双通道提取 ──→ 智能裁剪 ──→ 统一编号 fig1.png ...
              ├── 嵌入位图：原始分辨率无损
              └── 矢量图 cluster_drawings()：300 DPI 渲染
              └── Caption 自动匹配（Fig/Figure/图 关键词检测）
              └── 可选：灰度 + 对比度 1.4x → 专利级黑白线图
```

### 程序化图表生成

- **方法流程图** (graphviz)：从方法步骤自动生成，菱形判断节点 + 引用号 `(110)(120)...`
- **系统架构框图** (matplotlib)：FancyBboxPatch 组件块 + FancyArrowPatch 连接线 + 引用号
